...
import AddressDialog from '../Complementos/AddressDialog';
...
const ShippingQuoteForm = () => {
  ...
  const handleRequestQuote = async () => {
    ...
    try {
      ...
      const partnerOffers = shippingPartners.map(partner => {
        // Base price calculation using weight and number of packages
        const basePrice = 
        ...
        const insuranceCost = specialServices.insurance ? 
          (packageDetails.declaredValue * 0.05) : 0;
        const radCost = specialServices.RAD ? 80 : 0;
        const homeDeliverySurcharge = 
          specialServices.deliveryType === 'home' ? 50 : 0;
        
        // Calculate total price
        const totalPrice = basePrice + insuranceCost + radCost + homeDeliverySurcharge;
        
        return {
          partnerId: partner.id,
          partnerName: partner.name,
          partnerLogo: partner.logo,
          serviceType: partner.id === 'fedex' || partner.id === 'dhl' ? 'Express' : 'Standard',
  ...
  const handleConfirmShipping = () => {
    ...
    alert(`Su cotización ha sido confirmada. Recibirá un correo con los detalles de su envío con ${selectedService.partnerName}.`);
  ...
  const formatAddressForDisplay = (address) => {
    ...
    return (
      <>
        <Typography variant="body2">{address.street} {address.number}{address.interior ? `, Int. ${address.interior}` : ''}</Typography>
        <Typography variant="body2">Col. {address.colony}, {address.city}</Typography>
        <Typography variant="body2">{address.state}, CP {address.zipCode}</Typography>
        {address.reference && <Typography variant="body2" color="textSecondary">Ref: {address.reference}</Typography>}
      </>
    );
  };
  ...
  const formatAddressDestinationForDisplay = (destinationAddress) => {
    ...
    return (
      <>
        <Typography variant="body2">{destinationAddress.calle} {destinationAddress.numeroExterior}{destinationAddress.numeroInterior ? `, Int. ${destinationAddress.numeroInterior}` : ''}</Typography>
        <Typography variant="body2">Col. {destinationAddress.colonia}, {destinationAddress.ciudad}</Typography>
        <Typography variant="body2">{destinationAddress.estado}, CP {destinationAddress.cp}</Typography>
        {destinationAddress.referencia && <Typography variant="body2" color="textSecondary">Ref: {destinationAddress.referencia}</Typography>}
      </>
    );
  };
          ...
          <Grid container spacing={3}>
                    ...
                    >
                      {availableAddresses.map((address) => (
                        <MenuItem key={address.id} value={address.id}>
                          {address.street} {address.number}, {address.city} ({address.addressType})
                        </MenuItem>
                      ))}
                    </TextField>
                  </FormControl>
            ...
            <Grid item xs={12} md={6}>
              <Card sx={{ height: '100%' }}>
                  ...
                  action={
                    destinationAddress && (
                      <IconButtonWithTooltip
                        ...
                        onIconClick={() => handleOpenAddressDialog('destination', destinationAddress)}
                      />
                    )
                  }
                />
                <Divider />
                <CardContent>
                  <Box sx={{ p: 2, border: '1px solid #e0e0e0', borderRadius: 1, bgcolor: '#f5f5f5', mb: 2 }}>
                    {formatAddressDestinationForDisplay(destinationAddress)}
                  </Box>
                  
                  <Button
                    variant={!destinationAddress ? "contained" : "outlined"}
                    startIcon={<AddIcon />}
                    color="primary"
                    onClick={() => handleOpenAddressDialog('destination')}
                    fullWidth
                  >
                    {!destinationAddress ? "Agregar Dirección de Destino" : "Cambiar Dirección de Destino"}
                  </Button>
                </CardContent>
              </Card>
            </Grid>
              ...
              <Card>
                ...
                <CardContent>
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12}>
                      <Typography variant="subtitle1" sx={{ mb: 1 }}>Tipo de Entrega</Typography>
                      <RadioGroup
                        name="deliveryType"
                        value={specialServices.deliveryType}
                        onChange={handleSpecialServicesChange}
                      >
                        <FormControlLabel value="home" control={<Radio />} label="Entrega a Domicilio" />
                        <FormControlLabel value="branch" control={<Radio />} label="Entrega en Sucursal" />
                      </RadioGroup>
                    </Grid>
                  </Grid>
                </CardContent>
                ...
                <Card>
                  <CardContent>
                    <Typography color="error">
                      {quoteResults?.error || "Error al obtener cotizaciones. Intente nuevamente."}
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
                ...
                <CardContent>
                  <Box sx={{ textAlign: 'center', mb: 4 }}>
                    ...
                    <Typography variant="body1">
                      Ha seleccionado el servicio de {selectedService?.partnerName} ({selectedService?.serviceType})
                    </Typography>
                    <Typography variant="h6" color="primary" sx={{ mt: 1 }}>
                      ${selectedService?.price.toFixed(2)} MXN
                    </Typography>
                    <Typography variant="body2" color="textSecondary">
                      Tiempo estimado de entrega: {selectedService?.estimatedDelivery}
                    </Typography>
                  </Box>
                  ...
                  <Grid container spacing={3}>
                    <Grid item xs={12} md={6}>
                      <Paper variant="outlined" sx={{ p: 2 }}>
                        <Typography variant="subtitle1" gutterBottom>Dirección de Origen</Typography>
                        {formatAddressForDisplay(originAddress)}
                      </Paper>
                    </Grid>
                    ...
                    <Grid item xs={12} md={6}>
                      <Paper variant="outlined" sx={{ p: 2 }}>
                        <Typography variant="subtitle1" gutterBottom>Dirección de Destino</Typography>
                        {formatAddressDestinationForDisplay(destinationAddress)}
                      </Paper>
                    </Grid>
                    ...
                    <Grid item xs={12} md={6}>
                      <Paper variant="outlined" sx={{ p: 2 }}>
                        <Typography variant="subtitle1" gutterBottom>Detalles del Paquete</Typography>
                        <Typography variant="body2">Número de Bultos: {packageDetails.numberOfPackages}</Typography>
                        <Typography variant="body2">Peso Total: {packageDetails.weight} kg</Typography>
                        <Typography variant="body2">Contenido: {packageDetails.contents || 'No especificado'}</Typography>
                        <Typography variant="body2">Valor Declarado: ${packageDetails.declaredValue.toFixed(2)} MXN</Typography>
                      </Paper>
                    </Grid>
                    
                    <Grid item xs={12} md={6}>
                      <Paper variant="outlined" sx={{ p: 2 }}>
                        <Typography variant="subtitle1" gutterBottom>Servicios Adicionales</Typography>
                        <Typography variant="body2">Seguro de Envío: {specialServices.insurance ? 'Sí' : 'No'}</Typography>
                        <Typography variant="body2">Retorno de Acuse (RAD): {specialServices.RAD ? 'Sí' : 'No'}</Typography>
                        <Typography variant="body2">Tipo de Entrega: {specialServices.deliveryType === 'home' ? 'A Domicilio' : 'En Sucursal'}</Typography>
                      </Paper>
                    </Grid>
                  </Grid>
                  ...
                  <Box sx={{ mt: 4, textAlign: 'center' }}>
                    <Typography variant="body2" color="textSecondary" gutterBottom>
                      Al confirmar, recibirá un correo electrónico con los detalles de su envío y las instrucciones para proceder con el pago.
                    </Typography>
